...
LANG = {
 ...
 'sql':['sql'],'json':['json'],'yaml':['yml','yaml'],'markdown':['md'],'dart':['dart'],'lua':['lua'],'r':['r'],'bash':['sh'],'powershell':['ps1'],'solidity':['sol'],'gdscript':['gd'],'hlsl':['hlsl'],'glsl':['glsl'],'shaderlab':['shader'],'vue':['vue'],'svelte':['svelte'],'react':['jsx'],'angular':['ts'],'dockerfile':['dockerfile'],'graphql':['graphql'],'toml':['toml'],'xml':['xml'],'ini':['ini'],'scala':['scala'],'elixir':['ex','exs'],'perl':['pl','pm'],'zig':['zig'],'nim':['nim'],'matlab':['m'],'objective-c':['m','mm']}

CAPABILITIES = {
 'engineering': ['project architecture','code generation templates','file inspection','language classification','validation','build manifests','reproducible ZIP exports','multi-language project planning','dependency manifests','API scaffolding','test scaffolding'],
 ...
 'language_families': ['web','backend','systems','mobile','game','data','scripting','shader','configuration','query'],
...
@app.get('/languages')
def languages(): return {'ok':True,'languages':sorted(LANG.keys()),'count':len(LANG),'note':'Language coverage means Haven can classify, plan and scaffold these ecosystems. Open-ended generation requires the optional server-side model adapter.'}
...
def chat(c:Chat):
    ...
    elif 'language' in q or 'programming' in q or 'code' in q: reply='The Haven has a multi-language catalog covering web, backend, systems, mobile, game, data, scripting, shader and configuration ecosystems. Use /languages for the complete catalog. The current built-in generator is deterministic; a server-side model adapter is required for open-ended novel code generation.'
